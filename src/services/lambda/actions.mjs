...
export const sendPublic = async (payload, connectionId) => {
  await sendToAll(Object.keys(NAMES_DB), { publicMessage: `${NAMES_DB[connectionId]}: ${payload.message}` })
  return {};
};

export const sendPrivate = async (payload, connectionId) => {
  const to = Object.keys(NAMES_DB).find(key => NAMES_DB[key] === payload.to);
  await sendToOne(to, { privateMessage: `${NAMES_DB[connectionId]}: ${payload.message}` });
  return {};
};
